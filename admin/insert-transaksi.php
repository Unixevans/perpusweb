...
if($buku=="" || $dapat == "") {
	...
	include "../conn.php";
    $jum = 1;
	$query=mysqli_query($conn, "SELECT * FROM data_buku WHERE id = '$id'");
	...
	if ($tot == 0) {
		...
		$qt			= mysqli_query($conn, "INSERT INTO trans_pinjam VALUES (null, '$buku', '$id', '$nama', '$tgl_pinjam', '$tgl_kembali', 'pinjam', '$ket')") or die ("Gagal Masuk".mysqli_error($conn));

		$qu			= mysqli_query($conn, "UPDATE data_buku SET jum_temp=(jum_temp-1) WHERE id=$id ");
